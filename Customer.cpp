...
/// @param decorationType Type of decoration ("arrangement", "giftwrapping", "decorativepot").
...
TEST_SUITE("Customer")
{
	...
	TEST_CASE("addDecoration: arrangement increases price by 150")
	{
		...
		std::string cart = customer.cartToString();
		CHECK(cart.find("Rose") != std::string::npos);
		CHECK(cart.find(std::to_string(int(originalPrice + 150))) != std::string::npos);
	}
	...
	TEST_CASE("addDecoration: giftwrapping increases price by 100")
	{
		...
		std::string cart = customer.cartToString();
		CHECK(cart.find(std::to_string(int(originalPrice + 100))) != std::string::npos);
	}
	...
	TEST_CASE("addDecoration: decorativepot increases price by 200")
	{
		...
		std::string cart = customer.cartToString();
		CHECK(cart.find(std::to_string(int(originalPrice + 200))) != std::string::npos);
	}
