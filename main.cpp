...
static void applyCareToGarden(UserGarden& ug, CareStrategy* strategy) {
    if (!strategy) return;
    
    // Determine if strategy matches garden type
    bool isCorrectStrategy = false;
    std::string strategyName;
    
    if (dynamic_cast<SunnyCare*>(strategy)) {
        strategyName = "Sunny Care";
        isCorrectStrategy = (ug.type == "Sunny");
    } else if (dynamic_cast<PartialSunCare*>(strategy)) {
        strategyName = "Partial Sun Care";
        isCorrectStrategy = (ug.type == "Partial Sun");
    } else if (dynamic_cast<ShadyCare*>(strategy)) {
        strategyName = "Shady Care";
        isCorrectStrategy = (ug.type == "Shady");
    }
    
    std::cout << "\nApplying " << strategyName << " to " << ug.name << " (" << ug.type << ")...\n";
    
    if (!isCorrectStrategy) {
        std::cout << "Warning: This care strategy doesn't match the garden type!\n";
        std::cout << "Plants will not grow optimally.\n";
        return;
    }
    ...
    while (!it->isDone()) {
        ...
        if (p) {
            p->applyCare();           
            std::cout << "  - Cared for " << plantTypeName(p) 
                      << " (state: " << getPlantStateName(p) << ")\n";
            plantsCared++;
        }
        it->next();
    ...
    if (plantsCared == 0) {
        std::cout << "No plants to care for in this garden.\n";
    } else {
        std::cout << "Successfully cared for " << plantsCared << " plant(s)!\n";
    }
}
    ...
    while (running && actionsRemaining > 0) {
        ...
        switch (choice) {
            ...
            case 4: {
                ...
                CareStrategy* strategy = selectCareStrategy();
                if (!strategy) {
                    std::cout << "Invalid care strategy selection.\n";
                    pressEnter();
                    break;
                }
                
                applyCareToGarden(gardens[gix], strategy);
                actionsRemaining--;
                pressEnter();
                break;
            }
            case 5: { 
                std::cout << "\nHire which staff?\n";
                std::cout << "  1) Plant Caretaker\n";
                std::cout << "  2) Customer Assistant\n";
                std::cout << "Enter choice: ";
